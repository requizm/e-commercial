...
const defaultFields = { name: '', description: '', price: 5, image: '', category: 0 };
export function UpdateProduct() {
    ...
    function validateForm(): boolean {
        return isNullOrEmpty(fields.name) || isNullOrEmpty(fields.description) || isNullOrEmpty(fields.image) || fields.price <= 0;
    }

    async function handleSubmit(event: FormEvent<HTMLFormElement>) {
        event.preventDefault();
        const data = {
            id: id, name: fields.name, description: fields.description, price: fields.price, image: fields.image, category: {
                id: fields.category
            }
        };
    ...
    async function updateCategoryList() {
        ...
        setFields(prevState => ({ ...prevState, name: product.name, description: product.description, price: product.price, image: product.image, category: product.category.id }));
    }
        ...
        <form className="product-form" onSubmit={handleSubmit}>
            <div className="product-form-block">
                <div className="input-block name-block">
                    ...
                    <input type="text" placeholder="Enter" name="name" value={fields.name} className="text-input name-input" autoComplete="off" onChange={e => handleInput(e)} required />
                </div>
                <div className="input-block description-block">
                    <div className="description-text">Description*</div>
                    <input type="text" placeholder="Enter" name="description" value={fields.description} className="text-input description-input" autoComplete="off" onChange={e => handleInput(e)} required />
                </div>
                <div className="input-block price-block">
                    <div className="price-text">Price*</div>
                    <input type="number" placeholder="Enter" name="price" value={fields.price} className="text-input price-input" autoComplete="off" onChange={e => handleInput(e)} required />
                </div>
                <div className="input-block image-block">
                    <div className="image-text">Image*</div>
                    <input type="text" placeholder="Enter" name="image" value={fields.image} className="text-input image-input" autoComplete="off" onChange={e => handleInput(e)} required />
                </div>
                <div className="input-block category-block">
                    <div className="category-text">Category</div>
                    <select name="category" value={fields.category} className="text-input category-select" autoComplete="off" onChange={e => handleSelectInput(e)}>
                        {
